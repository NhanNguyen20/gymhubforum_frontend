...
export async function fetchThreadsByTag(
  tag: string,
  limit: number = 10,
  page: number = 0
) {
  try {
    const response = await axios.get(
      `/thread/${tag}?limit=${limit}&page=${page}`
    );
...
export async function fetchThreadsUser(
  id: number,
  limit: number,
  page: number
) {
  try {
    const res = await axios.get(
      `thread/user-${id}?limit=${limit}&page=${page}`
    );
    console.log(
      `thread/user-${id}?limit=${limit}&page=${page}`,
      res.status,
...
export async function fetchPostsThread(
  id: number,
  limit?: number,
  page?: number
) {
  try {
    const res = await axios.get(
      `post/thread-${id}?limit=${limit}&page=${page}`
    );
...
export async function banMember(
  ...
  try {
    const res = await axios.patch(
      `mod/dashboard/mod-${modID}/ban/member-${memberID}?duration=${duration}&reason=${reason}`
    );
    console.log(res.status);
  } catch (error) {
    console.log(error);
...
export async function fetchProfileLatestPost(
  id: number,
  page: number,
  pageSize: number
) {
