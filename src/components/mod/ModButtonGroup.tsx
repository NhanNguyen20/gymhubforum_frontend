  ...
  const handleResolveStatus = async () => {
    try {
      if (isPost && threadID && postID) {
        ...
        const response = await resolvePendingPost(
          modID,
          postID,
          threadID,
          selectedStatus,
        ...
        if (response?.status === 200) {
          console.log(
            "Post resolved successfully with status:",
            selectedStatus
          );
          ...
          console.error(
            "Post resolution failed with status:",
            response?.status || "No status"
          );
          message.error(
            `Failed to resolve post: ${response?.status || "No status"}`
          );
        }
        ...
        const response = await resolvePendingThread(
          modID,
          threadID,
          category,
          selectedStatus,
        ...
        if (response?.status === 200) {
          console.log(
            "Thread resolved successfully with status:",
            selectedStatus
          );
          message.success(
            `Thread resolved as "${selectedStatus}" successfully!`
          );
          handleResolve(selectedStatus); // Remove it from the list if successful
        } else {
          console.error(
            "Thread resolution failed with status:",
            response?.status || "No status"
          );
          message.error(
            `Failed to resolve thread: ${response?.status || "No status"}`
          );
        }
      }
    } catch (error) {
